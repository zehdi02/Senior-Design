...
def aggregate_run_results():
    ...
    for i, metric in enumerate(metrics, 1):
        plt.subplot(2, 5, i)
        plt.plot(all_results['epoch'], all_results[metric], 'o', label=metric)  # Dots for each point
        plt.plot(all_results['epoch'], all_results[metric], 'o--', label=f'{metric} (smoothed)')  # Dotted line for smooth curve
        plt.legend()
        plt.title(metric)
        plt.xlabel('Epoch')
        plt.ylabel('Value')
